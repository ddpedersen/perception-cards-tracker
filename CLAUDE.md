...
- **0/36 sets** currently complete (e.g. MJ in Flight 6/8; Black Mamba Portrait 4/8; Kobe Shirt Pull 3/8). Most painting names confirmed by Derek (Kobe Shirt Pull, LeBron City Scape); the rest were inferred from design/folder naming.
- **Parallels:** vocabulary has 1 parallel (Legendary, 4 variants, no tier defaults yet). `parallel` set on 8 designs (Kobe ×5 + LeBron + 2 soccer); `variant` set on those + the 8 MJ in Flight designs, which alert "missing parallel" by design (50 flagged designs total incl. naming alerts). Pending from Derek: fuller parallel/variant vocabulary (+ default tiers), parallel assignments for MJ in Flight & Black Mamba Portrait groups; then sync parsing of key-conformant names (see `NAMING.md`).
...
```js
{
  ...
  artwork: 'Kobe Shirt Pull',    // source painting (artist cards) — drives per-painting sets; null elsewhere
